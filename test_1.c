...
void print_array( const u8 array[], const int size)
{
		return;
        int start = 0;
        printf("\n");
        while(start < size){
                printf("%02x",array[start]);
                start++;
        }
        printf("\n");
...
int  md5_main()
{
      ...
      int fd2  = open("./lic.tmp", O_RDWR|O_CREAT|O_APPEND);
          int fd1  = open("./lic", O_RDWR|O_CREAT);
      fp = fopen ("./lic.z", "rb");
...
u8 aes_key[36] = {"qofpciqngjf12145786359dplfcjgn9a"};

//u8 aes_key[36] {"qofpciqngjf12145786359dplfcjgn9a"};


u8 src_txt[16] = {'H','t','e','k','\n'};
u8 encry_txt[16] = {0};
u8 decry_txt[16] = {0};
u8 dest_txt[500] = {0};
...
u8 mac_from_lic[6]= {0xff,0xff,0xff,0xff,0xff,0xff};
unsigned int  mac_from_lic_tmp[6]= {0x0};
void hl_set_aes_key()
{
	int i = 0;
	for(i=0;i<6;i++)
	{
		if(i == 5)
			mac_from_lic[i] ^= mac_from_lic[0];
		else
			mac_from_lic[i] ^= mac_from_lic[i+1];
		
	}
	
	//print_array(mac_from_lic,6);
	
	memset(aes_key+26,0x0,6);
	
	//print_array(aes_key,32);
	
	memcpy(aes_key+26,mac_from_lic,6);
...
int do_function_main(unsigned char * addr)
{
	
	
		memset(encry_txt,0x0,sizeof(encry_txt));
		memset(decry_txt,0x0,sizeof(decry_txt));
		
		memset(src_txt,0x0,sizeof(src_txt));
		memset(dest_txt,0x0,sizeof(dest_txt));


	
	ctx = malloc(sizeof(struct crypto_aes_ctx));
	int loop_count = 0;

	int fd4 = open("./lic", O_RDWR|O_CREAT|O_APPEND);
	...
	sprintf(src,
	"Time:%04d-%02d-%02d %02d:%02d:%02d\nCompany:Htek\nOwnership:China-NanJing\nMac=%02x:%02x:%02x:%02x:%02x:%02x"
	,1900 + p->tm_year,1 + p->tm_mon,p->tm_mday,8 + p->tm_hour,p->tm_min,p->tm_sec,addr[0],addr[1],addr[2],addr[3],addr[4],addr[5]);
	
	//print_array(addr,6);
	
	//printf("\n%s\n",src);
	
	write(fd4,src,strlen(src));
	close(fd4);
	...
	int i = 0;
	int count = 0;
	int fd1 = open("./lic.tmp", O_RDWR|O_CREAT|O_APPEND);
	int fd3 = open("./lic", O_RDWR|O_CREAT|O_APPEND);
	
	read(fd3,buffer,sizeof(buffer));
	
	memcpy(buffer_tmp,strstr(buffer,"Mac="),21);
	strcat(buffer_tmp,"\n");
	
	sscanf(buffer_tmp,"Mac=%02x:%02x:%02x:%02x:%02x:%02x\n",&mac_from_lic_tmp[0],&mac_from_lic_tmp[1],
				&mac_from_lic_tmp[2],&mac_from_lic_tmp[3],&mac_from_lic_tmp[4],&mac_from_lic_tmp[5]);
	
	
	mac_from_lic[0] = (unsigned char)(mac_from_lic_tmp[0]);
	mac_from_lic[1] = (unsigned char)(mac_from_lic_tmp[1]);
	mac_from_lic[2] = (unsigned char)(mac_from_lic_tmp[2]);
	mac_from_lic[3] = (unsigned char)(mac_from_lic_tmp[3]);
	mac_from_lic[4] = (unsigned char)(mac_from_lic_tmp[4]);
	mac_from_lic[5] = (unsigned char)(mac_from_lic_tmp[5]);
	...
	int fd2 = open("./licen", O_RDWR|O_CREAT|O_APPEND);
	
	write(fd2,buffer_tmp,strlen(buffer_tmp));
	
	memset(dest_txt,0x0,sizeof(dest_txt));
	

	crypto_aes_set_key(ctx, aes_key, 32);
	
	//return 0;
	while(1)
	{
		
		memset(src_txt,0x0,16);

		i = read(fd1,src_txt,16);
		if(i<=0)
			break;
		
		memset(encry_txt,0x0,16);
		memset(decry_txt,0x0,16);

	
       aes_encrypt(ctx,encry_txt,src_txt);

		
	  write(fd2,encry_txt,16);
	...
	close(fd2);
	close(fd1);
	unsigned char cmd[66];
	memset(cmd,0x0,66);
	
	sprintf(cmd,
	"cp ./licen ./licen_output/lic_%02x%02x%02x%02x%02x%02x_en",addr[0],addr[1],addr[2],addr[3],addr[4],addr[5]);
	//printf("wade : cmd : %s \n",cmd);
...
int mac_increse(unsigned char * mac)
{
	...
	memcpy(&num,mac_tmp,6);
	
	if(num == 0xffffffffffff)
	{
		printf("cannot increse mac addr : ff:ff:ff:ff:ff:ff");
...
int  main(int argc, char **argv)
{
	...
	DIR *dp;
	dp = opendir("./licen_output");

	if (dp == NULL) {
	
		mkdir("./licen_output",0777);
		}
	if(argc <= 1)
	{ 
        printf("bad\n");
	...
	{
		printf("wade wade use range\n");
		unsigned char  mac_start[6];
		unsigned char  mac_end[6];
		unsigned int   mac_from_lic_tmp[6];
		
		memset(mac_start,0x0,sizeof(mac_start));
		memset(mac_end,0x0,sizeof(mac_end));
		memset(mac_from_lic_tmp,0x0,sizeof(mac_from_lic_tmp));

#if 1
		if((isValidMacAddress(argv[2])!=1)||(isValidMacAddress(argv[3])!=1))
		{
		...
		sscanf(argv[2],"%02x:%02x:%02x:%02x:%02x:%02x\n",&mac_from_lic_tmp[0],&mac_from_lic_tmp[1],
			...
			mac_start[0] = (unsigned char)(mac_from_lic_tmp[0]);
			mac_start[1] = (unsigned char)(mac_from_lic_tmp[1]);
			mac_start[2] = (unsigned char)(mac_from_lic_tmp[2]);
			mac_start[3] = (unsigned char)(mac_from_lic_tmp[3]);
			mac_start[4] = (unsigned char)(mac_from_lic_tmp[4]);
			mac_start[5] = (unsigned char)(mac_from_lic_tmp[5]);
		...
		sscanf(argv[3],"%02x:%02x:%02x:%02x:%02x:%02x\n",&mac_from_lic_tmp[0],&mac_from_lic_tmp[1],
			...
			mac_end[0] = (unsigned char)(mac_from_lic_tmp[0]);
			mac_end[1] = (unsigned char)(mac_from_lic_tmp[1]);
			mac_end[2] = (unsigned char)(mac_from_lic_tmp[2]);
			mac_end[3] = (unsigned char)(mac_from_lic_tmp[3]);
			mac_end[4] = (unsigned char)(mac_from_lic_tmp[4]);
			mac_end[5] = (unsigned char)(mac_from_lic_tmp[5]);
#endif	
	
	
			print_array(mac_start,6);
			print_array(mac_end,6);
			int i=0,j=0;
			while(1)
				{
	
					do_function_main(mac_start);

        
					if(memcmp(mac_start,mac_end,6) == 0)	 
						{	
	...
	else if(strcmp(argv[1],"single") == 0)
	{
		...
		unsigned char  mac_used[6];
		unsigned int   mac_from_lic_tmp[6];
		

		memset(mac_used,0x0,sizeof(mac_used));
		memset(mac_from_lic_tmp,0x0,sizeof(mac_from_lic_tmp));
		...
		while(1)
		{
			...
			if((isValidMacAddress(argv[i+2])!=1))
				{
					printf("mac is not valid \n");
			...
			sscanf(argv[i+2],"%02x:%02x:%02x:%02x:%02x:%02x\n",&mac_from_lic_tmp[0],&mac_from_lic_tmp[1],
				&mac_from_lic_tmp[2],&mac_from_lic_tmp[3],&mac_from_lic_tmp[4],&mac_from_lic_tmp[5]);
		
			mac_used[0] = (unsigned char)(mac_from_lic_tmp[0]);
			mac_used[1] = (unsigned char)(mac_from_lic_tmp[1]);
			mac_used[2] = (unsigned char)(mac_from_lic_tmp[2]);
			mac_used[3] = (unsigned char)(mac_from_lic_tmp[3]);
			mac_used[4] = (unsigned char)(mac_from_lic_tmp[4]);
			mac_used[5] = (unsigned char)(mac_from_lic_tmp[5]);
		
			printf("%x %x %x %x %x %x\n",mac_used[0],mac_used[1],mac_used[2],mac_used[3],mac_used[4],mac_used[5]);
			
			do_function_main(mac_used);
